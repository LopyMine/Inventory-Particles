...
public class InventoryParticle extends AbstractInventoryElement<InventoryParticle> {
	...
	public static InventoryParticle create(ParticleConfig config, ParticleSpawnContext context) {
		...
		IInventoryElementTextureProvider textureProvider = IInventoryElementTextureProvider.getTextureProvider(config, config.getTextures(), config.getAnimationSpeed(), config.getLifeTimeTicks(), config.getAnimationType());
