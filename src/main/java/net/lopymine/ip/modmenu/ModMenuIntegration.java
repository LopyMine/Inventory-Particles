...
public class ModMenuIntegration implements ModMenuApi {
	...
	public ConfigScreenFactory<?> getModConfigScreenFactory() {
		...
		if (fabricLoader.isModLoaded("yet_another_config_lib_v3")) {
			...
			try {
				Version requestsVersion = Version.parse(InventoryParticles.YACL_DEPEND_VERSION);
				if (version.compareTo(requestsVersion) >= 0) {
					return YACLConfigurationScreen::createScreen;
				}
			} catch (VersionParsingException e) {
				InventoryParticlesClient.LOGGER.error("Failed to compare YACL version, tell mod author about this error: ", e);
			}
